...
st.set_page_config(**config.get_streamlit_config_dict())
...
if not st.session_state.username:
    ...
    if app_mode == "聊天學習":
            ...
            with button_container:
                ...
                with col1:
                    if st.button("➕", key="new_chat_button", help="新增聊天"):
                        ...
                        new_chat_name = f"{chat_config.default_chat_name} {chat_count + 1}"
                    ...
                    if st.button("🗑️", key="delete_chat_button", help="刪除目前的聊天"):
                        if db.delete_chat_session(selected_chat):
                            ...
                            if remaining_chats:
                                ...
                                db.create_chat_session(st.session_state.user_id, f"{chat_config.default_chat_name} 1", new_chat_id)
                                st.session_state.current_chat_id = new_chat_id
        ...
        if not current_chat_messages:
            # 從配置獲取歡迎訊息
            chat_config = get_chat_config()
            welcome_msg = chat_config.welcome_message
